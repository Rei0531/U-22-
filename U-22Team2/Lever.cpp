...
int Lever(void) {
	...
	if ((g_Player.x > 0) & (g_Player.x < 200) & (g_Pad.KEY_B == TRUE)
		& (gim.SwitchFlag == 0) & (gim.SwitchWait == 0) & (g_Player.NowColor == BLUE)) { //スイッチフラグがOFFであり待機時間が０でありスイッチと重なってあり
		...
		& (gim.SwitchFlag == 1) & (gim.SwitchWait == 0) & (g_Player.NowColor == BLUE)) {  //レバーがONまたはOFFになった場合待機時間が加わる
		gim.SwitchFlag = 0;
		gim.SwitchWait = 10;
	}
